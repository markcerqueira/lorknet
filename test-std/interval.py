...
def boiler(filename, mode):
	...
	for line in open(filename,'r').readlines():
		...
		if (first_time == 1):
			...
			interval = float(elements[3]);

			first_packet = float(elements[1]);
			...
			last_ID = float(elements[1]);

		else:
			current_ID = float(elements[1]);

			interval = float(elements[3])/(current_ID - last_ID);
			average = average + interval;
	...
	for line in open(filename,'r').readlines():
		...
		if (first_time == 1):
			first_time = 0;
			stddev = stddev + math.pow(float(elements[3]) - average, 2);
			last_ID = float(elements[1]);

		else:
			current_ID = float(elements[1]);
			stddev = stddev + math.pow(float(elements[3])/(current_ID - last_ID) - average, 2);
			last_ID = current_ID;
	...
	re.write("Average of interval: " + str(round(average, 2)) + " ms\n");
        ...
        if (mode == "UNICAST"):
          ...
          text(packets_expected * .78, 33, summary, color = 'g', weight = 'bold', size = '10', backgroundcolor = 'w')
        ...
        if (mode == "MULTICAST"):
          ...
          summary += str(round(average, 2)) + " ms\n "
          summary += str(round(stddev, 2)) + " ms"
          text(packets_expected * .90, 33, summary, color = 'b', weight = 'bold', size = '10', backgroundcolor = 'w')

#------------------------------------------------------------------------------

# the program name, target name, rate, SSID, number of machines, number of servers
if len(sys.argv) != 7: 
  sys.exit("interval.py: python interval.py your_name target_name rate_ms SSID machine_count server_count")
...
text(packets_expected * .61, 33, summary, color = 'k', weight = 'extra bold', size = '10', backgroundcolor = 'w')
