...
def boiler(filename, mode):
	...
	for line in open(filename,'r').readlines():
		...
		if (first_time == 1):
			...
			interval = float(elements[5]);

			first_packet = float(elements[3]);
			...
			if (math.fabs(float(elements[3]) - float(elements[2])) < GT):
				within_GT += 1;

			travel_avg = travel_avg + float(elements[4]);
			...
			interval = float(elements[5])/(current_ID - last_ID);
			average = average + interval;
	...
	for line in open(filename,'r').readlines():
		...
		if (first_time == 1):
			first_time = 0;
			travel_stddev = travel_stddev + math.pow(float(elements[4]) - travel_avg, 2);
			stddev = stddev + math.pow(float(elements[5]) - average, 2);
			last_ID = float(elements[1]);

		else:
			current_ID = float(elements[1]);
			travel_stddev = travel_stddev + math.pow(float(elements[4]) - travel_avg, 2);
			stddev = stddev + math.pow(float(elements[5])/(current_ID - last_ID) - average, 2);
			last_ID = current_ID;
	...
	re.write("Average of latency: " + str(round(travel_avg, 2)) + " ms\n");
        ...
        # prepare summary string for plotting on the graph
